  ...
  return (
    <div className="cap-landing antialiased">
      ...
      <main className="max-w-7xl mx-auto px-6 pt-16 pb-24 overflow-hidden">
        ...
        <section id="faq" className="mb-32 max-w-3xl mx-auto gsap-reveal relative z-10"><h2 className="text-4xl font-black text-center mb-12">Frequently Asked Questions</h2><div className="space-y-4">{faqs.map((faq, i) => <div key={faq.q} className={`faq-item bg-[#fffef7] neo-border neo-shadow rounded-xl overflow-hidden ${activeFaq === i ? "active" : ""}`}><button type="button" onClick={() => setActiveFaq(activeFaq === i ? null : i)} aria-expanded={activeFaq === i} className="w-full p-6 flex justify-between items-center bg-white hover:bg-gray-50 transition-colors text-left cursor-pointer"><h4 className="text-xl font-bold">{faq.q}</h4><div className="faq-icon transition-transform duration-300 w-8 h-8 rounded-full border-2 border-[#1a1a2e] flex items-center justify-center font-bold text-xl shrink-0">+</div></button><div className="faq-content px-6 text-lg font-medium text-[#1a1a2e]/80 bg-white">{faq.a}</div></div>)}</div></section>
      </main>
